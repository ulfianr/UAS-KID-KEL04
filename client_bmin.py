...
# 2. SIMPAN PUBLIC KEY KE PEM (Untuk diupload ke /store)
client_pub_pem = pub_client_key.public_bytes(
    encoding=serialization.Encoding.PEM,
    format=serialization.PublicFormat.SubjectPublicKeyInfo
)
with open("pub_client.pem", "wb") as f:
    f.write(client_pub_pem)
# --- HITUNG HASH UNTUK /store ---
store_expected_hash = hashlib.sha256(client_pub_pem).hexdigest()
...
payload = {
    ...
    "certificate": b64(cert_pem),          # TAMBAHKAN INI
    "client_public_key": b64(client_pub_pem), # TAMBAHKAN INI
    "ephemeral_public_key": b64(eph_pub_pem),
    "nonce": b64(nonce),
    "ciphertext": ciphertext_b64_str,
    "signature_ed25519": b64(signature_ed25519),
    "signature_ecdsa": b64(signature_ecdsa),
    "ciphertext_hash": relay_ciphertext_hash 
}

print("Payload siap dikirim ke server:")
print(f"1. Hash untuk /store  : {store_expected_hash}")
print(f"2. Hash untuk /verify : {verify_message_hash}")
print(f"3. Hash untuk /relay  : {relay_ciphertext_hash}")
